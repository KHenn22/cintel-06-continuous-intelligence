...
Paths (relative to repo root)
    ...
    INPUT FILE: data/ontime_reporting_hennelly.csv
    OUTPUT FILE: artifacts/ontime_assessment_hennelly.csv
...
DATA_FILE: Final[Path] = (
    DATA_DIR / "stl_2025_ontime.csv"
)  # use data for specific airport ontime data being analyzed
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "ontime_assessment_hennelly.csv"
...
MIN_STABLE_ONTIME: Final[float] = 80.0
MIN_ACCEPTABLE_ONTIME: Final[float] = 70.0
...
def main() -> None:
    ...
    df = df.with_columns(
        [(pl.col("ARR_DELAY_NEW") <= 15).cast(pl.Int32).alias("is_ontime")]
    )

    summary_df = (
        df.group_by(["MONTH"])
        .agg([(pl.col("is_ontime").mean() * 100).alias("pct_ontime_arr")])
        .sort("MONTH")
    )
    ...
    anomalies_df = summary_df.filter(pl.col("pct_ontime_arr") < MIN_ACCEPTABLE_ONTIME)
    LOG.info(
        f"STEP 3. Using thresholds: MIN_ACCEPTABLE_ONTIME={MIN_ACCEPTABLE_ONTIME}, "
        f"MIN_STABLE_ONTIME={MIN_STABLE_ONTIME}"
    )
    ...
    # Three tier assessment label: STABLE (>=80), WARNING (<80 but >=70), or DEGRADED (<70)
    summary_df = summary_df.with_columns(
        pl.when(pl.col("pct_ontime_arr") < MIN_ACCEPTABLE_ONTIME)
        .then(pl.lit("DEGRADED"))
        .when(pl.col("pct_ontime_arr") < MIN_STABLE_ONTIME)
        .then(pl.lit("WARNING"))
