"""
continuous_intelligence_hennelly.py - Project script (example).
...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/system_assessment_hennelly.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.continuous_intelligence_hennelly
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_hennelly.csv"
...
MAX_DEGRADED_ERROR_RATE: Final[float] = 0.05
MAX_DEGRADED_AVG_LATENCY: Final[float] = 40.0
MAX_WARN_ERROR_RATE: Final[float] = 0.02
MAX_WARN_AVG_LATENCY: Final[float] = 20.0
...
def main() -> None:
    ...
    anomalies_df = df.filter(
        (pl.col("error_rate") > MAX_DEGRADED_ERROR_RATE)
        | (pl.col("avg_latency_ms") > MAX_DEGRADED_AVG_LATENCY)
    )
    LOG.info(
        f"STEP 3. Using thresholds: MAX_DEGRADED_ERROR_RATE={MAX_DEGRADED_ERROR_RATE}, "
        f"MAX_DEGRADED_AVG_LATENCY={MAX_DEGRADED_AVG_LATENCY},"
        f"MAX_WARN_ERROR_RATE={MAX_WARN_ERROR_RATE},"
        f"MAX_WARN_AVG_LATENCY={MAX_WARN_AVG_LATENCY},"
    )
    ...
    # Add a three tier assessment label: STABLE, WARNING, or DEGRADED
    summary_df = summary_df.with_columns(
        pl.when(
            (pl.col("avg_error_rate") > MAX_DEGRADED_ERROR_RATE)
            | (pl.col("avg_latency_ms") > MAX_DEGRADED_AVG_LATENCY)
        )
        .then(pl.lit("DEGRADED"))
        .when(
            (pl.col("avg_error_rate") > MAX_WARN_ERROR_RATE)
            | (pl.col("avg_latency_ms") > MAX_WARN_AVG_LATENCY)
        )
        .then(pl.lit("WARNING"))
